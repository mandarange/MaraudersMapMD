...
> [AI RULE] 우리는 AI(LLM)를 제공하지 않는다. Cursor/Claude/기타 AI 환경이 레포를 읽을 때 더 정확히/더 적은 토큰으로 이해하게 만드는 **"파일 기반 산출물"** 을 제공한다.
...
요즘 개발은 Cursor, Anthropic Claude, 기타 AI IDE/Agent 환경과 함께 진행되는 경우가 많다. 그 결과 PRD/ADR/RFC/운영 가이드 등 Markdown 문서 작성량이 폭증했다.

하지만 AI는 토큰 제한 때문에 큰 문서를 통째로 읽지 못하고, 검색/부분 조회를 반복하면서 시간이 늘고 정확도가 떨어진다.
...
문서를 구조화하고, 토큰 친화적 산출물(AI Map / Section Pack / Index)을 생성한다.
...
| 타이핑 후 프리뷰 갱신(일반) | 디바운스 200ms + 렌더 ≤ 80ms |
| 대량 붙여넣기(50KB~100KB) | 프리뷰 업데이트 최대 1~2회, 에디터 프리징 없음 |
...
> [AI RULE] 우리는 AI를 제공하지 않는다.

1. **Vendor-neutral / file-first**: `docs/MaraudersMap/` 산출물로 외부 AI 도구가 "읽기만 하면" 성능 개선
2. **Token-aware**: 섹션 분해 / 지도 / 인덱스 / 예산 기반 컨텍스트
3. **가벼움 우선**: React/Monaco/Chromium 번들/네이티브 DB 등 무거운 선택 지양
4. **저장 시 생성(onSave) 기본**: 타이핑 중 백그라운드 churn 최소화
...
**Acceptance**: 클릭 토글 후 커서/선택 영역이 비정상적으로 깨지지 않아야 함

### 6.4 이미지 워크플로우 (Images)

**목표**: 이미지가 깨지지 않고 (preview/pdf), 레포 구조가 정리되며, 사용이 즉시 가능해야 한다.

**기능(필수)**

| 기능 | 설명 |
|------|------|
| A) Insert Image from File | 파일 선택 → `${mdDir}/assets/`(기본)로 복사 → `![alt](./assets/...)` 삽입 |
| B) Drag & Drop into Preview | 프리뷰에 드롭 → assets 저장 → 링크 삽입 |
| C) Paste Image in Preview | 프리뷰 paste 이벤트로 이미지 blob 수신 → assets 저장 → 링크 삽입 |
| D) 원격 이미지(URL) | 기본 OFF(옵션 ON) |
...
#### 6.5.1 HTML Export (필수)

프리뷰 템플릿 재사용하여 HTML 출력

#### 6.5.2 PDF Export (필수)

> [AI RULE] 핵심 원칙: Chromium 번들 금지
...
### 6.7 AI Readability Support (Vendor-neutral / file-first)

> [AI RULE] 외부 AI 도구가 레포의 파일을 읽을 때, 토큰 제한에도 정확히 이해하도록 "AI용 파일 산출물"을 생성한다. 우리는 AI를 호출하거나 제공하지 않는다.

#### 6.7.1 AI artifacts 출력 위치(표준)

기본: 워크스페이스 루트 `docs/MaraudersMap/`
...
| `maraudersMapMd.ai.outputDir` | `docs/MaraudersMap` |
...
- `docs/MaraudersMap/<docId>/ai-map.md`만 읽어도 문서 구조/섹션 범위/토큰 추정이 파악 가능
...
| 리스크 | 대응 |
|--------|------|
| PDF가 시스템 브라우저에 의존 | auto-detect + 경로 설정 + HTML fallback |
| 대문서에서 생성물/히스토리 용량 증가 | gzip + retention + max storage + checkpoint 보호 |
| 특정 AI 도구 특화 요구 증가 | 벤더 중립 원칙 유지(파일/클립보드/규칙만), 특정 SDK 연동 제외 |
| 네이밍/브랜딩 리스크 | 원작 자산/로고 사용 금지, "fan-inspired" 고지(필요 시) |

---

## 12. GitHub / 오픈소스 운영 가이드 (권장)
