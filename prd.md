...
요즘 개발은 Cursor, Anthropic Claude, 기타 AI IDE/Agent 환경과 함께 진행되는 경우가 많고, 그 결과 PRD/ADR/RFC/운영 가이드 등 Markdown 문서 작성량이 폭증했다.
하지만 AI는 토큰 제한 때문에 큰 문서를 통째로 읽지 못하고, 검색/부분 조회를 반복하면서 시간이 늘고 정확도가 떨어지는 문제가 있다.
...
6.6.3 UI/동작(필수)
	...
	•	액션:
	•	View (읽기)
	•	Diff with Current (VS Code diff)
	•	Restore
	•	Copy Snapshot Text
	•	restore 전 보호 옵션:
...
핵심: 외부 AI 도구가 레포의 파일을 읽을 때, 토큰 제한에도 정확히 이해하도록 “AI용 파일 산출물”을 생성한다.
우리는 AI를 호출하거나 제공하지 않는다.
...
AI Artifacts
	...
	•	maraudersMapMd.ai.gitPolicy (ignoreAll | commitMapOnly | commitAll) default ignoreAll
...
8.2 “3-Plane Architecture”
	...
	•	VS Code native 편집 + WorkspaceEdit로 조작
...
AI Readability Support
	•	.ai/<docId>/ai-map.md만 읽어도 문서 구조/섹션 범위/토큰 추정이 파악 가능
...
12) GitHub/오픈소스 운영 가이드(권장)
	...
	•	팀 공유가 필요하면:
	•	ai-map.md만 커밋(옵션 정책 commitMapOnly)
